...
# # Proyecto Clasificación de aves Parte 4
# # Clasificación con LDA y QDA
...
lda.fit(dxt,dyt[:,0])
predicted=lda.predict(dxte)[None].T     #trying less features
...
def CrossV(X,Y,Folds=10,solver='svd',type_da='LDA',average='NO'):
    ...
    for i in range(Folds):
        ...
        lda.fit(xtrain,ytrain) #traines the nn
        tpredictions=lda.predict(xtrain) #predicts the same data used to train to see if we are overtraiing
        ...
        predictions=lda.predict(xtest) #clasifies the test folder
...
Etr,Et,F1tr,F1t=CrossV(dx,dy[:,0],solver='svd',type_da='LDA',average='NO')
Etr,Et,F1tr,F1t


# ### Grafico los Scores para las iteraciones de la validación
...
a=np.around(F1tr,3)
b=np.around(F1t,3)
np.savetxt('F1_TRAINING_LDA.txt',a,fmt='%.2f',newline='\n')
np.savetxt('F1_TEST_LDA.txt',b,fmt='%.2f',newline='\n')
...
Etr,Et,F1tr,F1t=CrossV(dx,dy[:,0],solver='svd',type_da='QDA',average='NO')
Etr,Et,F1tr,F1t
...
a=np.around(F1tr,3)
b=np.around(F1t,3)
np.savetxt('F1_TRAINING_QDA.txt',a,fmt='%.2f',newline='\n')
np.savetxt('F1_TEST_QDA.txt',b,fmt='%.2f',newline='\n')
